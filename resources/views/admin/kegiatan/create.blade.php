...
@section('judul', 'Tambah Kegiatan Adat')

@section('konten')
    <nav class="flex items-center gap-2 text-sm text-stone-500 mb-4">
        <a href="{{ route('admin.kegiatan.index') }}" class="hover:text-amber-800 transition">Kegiatan Adat</a>
    ...
    <div class="max-w-3xl">
        <form method="POST" action="{{ route('admin.kegiatan.store') }}" enctype="multipart/form-data"
            ...
            <div class="mt-6 flex gap-3 pt-4 border-t border-stone-100">
                ...
                <a href="{{ route('admin.kegiatan.index') }}" class="btn-outline">Batal</a>
            </div>
        </form>
    </div>
@endsection
